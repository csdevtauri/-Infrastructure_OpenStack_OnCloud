...
resource "openstack_networking_secgroup_rule_v2" "secgroup_openvpn_rule_tcp_v4" {
  direction         = "ingress"
  ethertype         = "IPv4"
  protocol          = "tcp"
  port_range_min    = 1194
...
resource "openstack_networking_secgroup_rule_v2" "secgroup_proxy_rule_http_v4" {
  direction         = "ingress"
  ethertype         = "IPv4"
  protocol          = "tcp"
  port_range_min    = 80
...
resource "openstack_networking_secgroup_rule_v2" "secgroup_proxy_rule_https_v4" {
  direction         = "ingress"
  ethertype         = "IPv4"
  protocol          = "tcp"
  port_range_min    = 443
